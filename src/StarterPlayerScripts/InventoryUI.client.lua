...
local function buildHotbar()
	...
	-- ─── Mana bar (above hotbar) ───────────────────────────────────────
	-- Same width as the hotbar, sits directly on top of it. Left-aligned
	-- icon, wooden background to match the hotbar, cyan fill with a
	-- centered text label showing the current mana amount. The label's
	-- X-position is tweened so it always sits in the middle of the
	-- currently filled portion of the bar, smoothly following the fill
	-- as ManaCurrent changes.
	local MANA_ICON_ASSET = "rbxassetid://131647230431306"
	local MANA_ICON_SIZE  = 48
	local MANA_BAR_GAP    = 8    -- vertical gap between mana bar and hotbar
	local MANA_BAR_HEIGHT = MANA_ICON_SIZE + 8
	local MANA_TRACK_LEFT = MANA_ICON_SIZE + 12   -- icon width + right gap
	local MANA_TRACK_RIGHT = 10
	local MANA_TRACK_HEIGHT = 26
	local MANA_TWEEN_TIME = 0.25

	local manaBar = Instance.new("Frame")
	manaBar.Name = "ManaBar"
	manaBar.Size = UDim2.new(0, barWidth, 0, MANA_BAR_HEIGHT)
	manaBar.Position = UDim2.new(
		0.5, -barWidth / 2,
		1, -(SLOT_SIZE + SLOT_PAD * 2) - 10 - MANA_BAR_HEIGHT - MANA_BAR_GAP
	)
	manaBar.BackgroundColor3 = COLORS.hotbarBg
	manaBar.BackgroundTransparency = 0.15
	manaBar.BorderSizePixel = 0
	manaBar.Parent = hotbarGui

	local manaBarCorner = Instance.new("UICorner")
	manaBarCorner.CornerRadius = UDim.new(0, 8)
	manaBarCorner.Parent = manaBar

	local manaBarStroke = Instance.new("UIStroke")
	manaBarStroke.Color = COLORS.panelBorder
	manaBarStroke.Thickness = 2
	...
	manaIcon.Position = UDim2.new(0, 4, 0.5, 0)
	...
	manaTrack.BackgroundColor3 = Color3.fromRGB(28, 48, 80)
	...
	manaTrackCorner.CornerRadius = UDim.new(0, 6)
	manaTrackCorner.Parent = manaTrack

	local manaTrackStroke = Instance.new("UIStroke")
	manaTrackStroke.Color = Color3.fromRGB(15, 30, 55)
	manaTrackStroke.Thickness = 1
	manaTrackStroke.Parent = manaTrack

	local manaFill = Instance.new("Frame")
	manaFill.Name = "ManaFill"
	manaFill.BackgroundColor3 = Color3.fromRGB(80, 160, 255)
	...
	-- Text label sits on the track (not on the fill) so it's never
	-- clipped, and its X anchor moves to sit over the middle of the
	-- filled portion. The Y is fixed to the vertical center.
	...
	manaText.Size = UDim2.fromOffset(80, 22)
	manaText.Text = "0"
	manaText.ZIndex = 2
	manaText.Parent = manaTrack

	-- Text needs to render OVER the fill (which is a sibling inside
	-- `manaTrack`). Raise the track's ChildrenZ behaviour by bumping
	-- the fill's ZIndex low and the text's ZIndex high.
	manaFill.ZIndex = 1
	...
	-- Subscribe to the replicated Characteristics mana values. First
	-- call paints the bar instantly (no tween), subsequent changes
	-- animate smoothly so increments/decrements are readable.
	task.spawn(function()
		...
		if not manaBar.Parent then return end  -- hotbar was rebuilt

		updateManaDisplay(current.Value, max.Value, false)

		local function refresh()
			if not manaBar.Parent then return end
