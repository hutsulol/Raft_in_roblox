...
-- 30 s timeouts on these WaitForChild calls silence Roblox's
-- "Infinite yield possible" warning during cold loads (the server
-- scripts that create the events occasionally lag behind client
-- init, especially on the first session after sync). We DO NOT
-- bail if any come back nil — the script keeps building its UI so
-- the new wood-palette workbench panel never silently reverts to
-- whatever older UI Studio might still have authored.
...
-- ─── Wires ──────────────────────────────────────────────────────────
-- Connect each event only when it actually exists. The 30 s
-- WaitForChild timeouts above mean any of these can be nil on a
-- cold-load misconfig — we just silently skip the wire instead of
-- erroring at script init, which kept the wood-palette UI visibly
-- alive even when one event hadn't replicated yet.
if openWorkbenchEvent then
	openWorkbenchEvent.OnClientEvent:Connect(function()
		if isOpen then closeUI() else openUI() end
	end)
else
	warn("[WorkbenchUI] OpenWorkbench event missing — workbench prompt won't open the UI")
end
